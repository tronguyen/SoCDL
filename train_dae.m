function [ M ] = train_dae(M, params, side)
...
%     if M.data.binary == 0
%         % make it zero-mean
%         patches_mean = mean(batchdata, 1);
%         batchdata = bsxfun(@minus, batchdata, patches_mean);
% 
%         % make it unit-variance
%         patches_std = std(batchdata, [], 1);
%         batchdata = bsxfun(@rdivide, batchdata, patches_std);
%     end
    v0 = batchdata;
    % add noise
    v0_clean = v0;
    ...
    if batch==1
        ...
        fprintf(2, '\t***norms: %f/%f/%f\n', ...
            M.W{1}(:)' * M.W{1}(:) / length(M.W{1}(:)), ...
            M.biases{1}' * M.biases{1} / length(M.biases{1}), ... % vbias
            M.biases{2}' * M.biases{2} / length(M.biases{2})); % hbias
    end
