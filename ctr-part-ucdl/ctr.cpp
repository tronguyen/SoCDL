...
void c_ctr::read_init_information(const char* delta_init_path) {
  ...
  FILE *f = fopen(delta_init_path, "r");
...
void c_ctr::init_model(int ctr_run) {
  ...
  if (ctr_run) {
    ...
    // gsl_matrix_memcpy(m_U, m_delta); --> mycode: do nothing
  }
  else {
...
void c_ctr::learn_map_estimate(const c_data* users, const c_data* items,
  ...
  while ((iter < param->max_iter and converge > 1e-7 ) or iter < min_iter) {
    ...
    if (iter % param->save_lag == 0) {
      ...
      sprintf(name, "%s/ctr-tmp/%04d-U.dat", directory, iter);
      FILE * file_U = fopen(name, "w");
      mtx_fprintf(file_U, m_U);
      fclose(file_U);

      sprintf(name, "%s/ctr-tmp/%04d-V.dat", directory, iter);
      FILE * file_V = fopen(name, "w");
      mtx_fprintf(file_V, m_V);
      fclose(file_V);

      sprintf(name, "%s/ctr-tmp/%04d-D.dat", directory, iter);
      FILE * file_D = fopen(name, "w");
      mtx_fprintf(file_D, m_delta);
      fclose(file_D);
    }
  }
  ...
  sprintf(name, "%s/final-D.dat", directory);
  FILE * file_D = fopen(name, "w");
  mtx_fprintf(file_D, m_delta);
  fclose(file_D);
