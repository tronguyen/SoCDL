function [ M ] = train_dae_from_ctr_doublefw(M, F, params, side)
...
%% Compute reconstruction error
    ...
    if M.data.binary
        rerr = -sum(sum(v0_clean .* log(max(vr, 1e-16)) + (1 - v0_clean) .* log(max(1 - vr, 1e-16)), 2));
    else
...
%         temp.grad.rb = 0 * (M.grad.rb)';
    ...
    deltae{end} = deltad{end}.*dsigmoid(h0e{end});
...
%         end
    ...
    % FW encode-part
    for l = 2:n_layers
        h0h{l} = bsxfun(@plus, h0h{l-1} * M.W{l-1}, M.biases{l}');

        if l < n_layers || M.bottleneck.binary
            h0h{l} = sigmoid(h0h{l}, M.hidden.use_tanh);
        end
        % add dropout
        if M.dropout~=0
            mask = binornd(1,1-M.dropout,size(h0h{l}));
            h0h{l} = h0h{l}.*mask;
            clear mask;
        end
    end
    ...
    if side ==2 && params.urelax
%         h0re = sigmoid(bsxfun(@plus, h0h{end} * M.rW, M.rb'), 2);
        h0rh = fw_relax(M, h0h{end}); % or softmax: 1=tanh
    end;
    ...
    if side ==2 && params.urelax % for user-side relax the mapping
        deltarh = (h0rh - v_v); %.*dsigmoid(h0rh); % *dsigmoid for sigmoid, softmax noneed
        deltah{end} = deltarh*M.rW';
        deltah{end} = deltah{end}.*dsigmoid(h0h{end}); % just add to be similar to non-relax
        
        temp.grad.rW = lenc/params.ln*...
            (h0h{end}'*deltarh)/(size(v0,1));
%         temp.grad.rb = lenc/params.ln*mean(deltarh,1);
    else
        deltah{end} = (h0h{end}-v_v).*dsigmoid(h0h{end});
    end;
...
%         end
        if l>1
            deltah{l} = deltah{l}.*dsigmoid(h0h{l+1},M.hidden.use_tanh);
        end
