function [ M ] = train_dae_from_ctr_withmask(M, params, side)
...
%     mymask = createmask(v0_clean, params);
    mymask = ones(size(v0_clean));
    hr = sdae_get_hidden(1, v0_clean, M);
    vr = sdae_get_visible(hr, M);
    if batch==1
        fprintf(1,'\t***Mean: H=%.9f Std: H= %.9f\r', mean(hr(:)), std(hr(:)));
    end
    
    if M.data.binary
        rerr = -sum(sum((v0_clean .* log(max(vr, 1e-16)) + (1 - v0_clean) .* log(max(1 - vr, 1e-16))).*mymask, 2));
    else
        rerr = sum(sum(((v0_clean - vr).*mymask).^2,2));
    end
    if params.use_gpu > 0
        rerr = gather(rerr);
    end
...
%         temp.grad.rb = 0 * (M.grad.rb)';
    ...
    deltad{1} = (h0d{1} - v0_clean).*mymask;%.*dsigmoid(h0d{1}, M.visible.use_tanh);
