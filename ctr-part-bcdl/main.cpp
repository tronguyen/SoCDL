...
int main(int argc, char* argv[]) {
  ...
  const char* const short_options = "hd:x:i:a:b:u:v:r:s:m:k:t:e:y:z:w:f";
  const struct option long_options[] = {
    ...
    {"delta_init",  required_argument, NULL, 'f'},
  ...
  if (delta_init_path != NULL){
    if (!file_exists(delta_init_path)) {
    printf("community proportions file %s doesn't exist! quit ...\n", delta_init_path);
    exit(-1);
    }
    printf("community proportions file: %s\n", delta_init_path); 
  } else{
    dae_run = 0;
  }
  ...
  ctr_param.set(a, b, lambda_u, lambda_v, learning_rate, alpha_smooth,
      random_seed, max_iter, save_lag, theta_opt, ctr_run, lda_regression, dae_run);
  ...
  if (mult_path != NULL) {
    ...
    ctr->read_init_information(delta_init_path, theta_init_path, beta_init_path, c, alpha_smooth);
  }
