...
struct ctr_hyperparameter {
  ...
  void set(double aa, double bb, 
           ...
           int to, int cr, int lda_r, int dr) {
    a = aa; b = bb; 
    ...
    ctr_run = cr; lda_regression = lda_r; dae_run = dr;
  }
...
public:
  c_ctr();
  ~c_ctr();
  void read_init_information(const char* delta_init_path,
  ...
  void init_model(int ctr_run, int dae_run);
